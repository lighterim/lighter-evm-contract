...
if [ -z "$TLSN_WITNESS_PRIV_KEY" ]; then
    echo "Error: TLSN_WITNESS_PRIV_KEY environment variable is not set"
    exit 1
fi

if [ -z "$TLSN_PROOF_VERIFIER" ]; then
    echo "Error: TLSN_PROOF_VERIFIER environment variable is not set"
    exit 1
fi
...
export tlsnWitnessPrivKey=${TLSN_WITNESS_PRIV_KEY}
...
export confirmTs=$(date +%s)
export verifierEscrowTypedHash=$(cast call $TlsnProofVerifier "getEscrowTypedHash((uint256,address,uint256,uint256,uint256,address,address,uint256,bytes32,bytes32,bytes32,address,uint256))" "$escrowParms")
echo "verifierEscrowTypedHash: $verifierEscrowTypedHash \n escrowParms: $escrowParms"
export relayerSig=$(cast wallet sign --no-hash $verifierEscrowTypedHash --private-key=$relayerPrivKey)
echo "relayerSig: $relayerSig"
export paymentParams="($paymentMethod, $paymentId, $payeeDetails, $paymentAmount, $currency, $confirmTs, $tradeId)"
export paymentTypedHash=$(cast call $TlsnProofVerifier "getPaymentTypedHash((bytes32,bytes32,bytes32,uint256,bytes32,uint64,uint256))" "$paymentParams")
echo "paymentTypedHash: $paymentTypedHash \n paymentParams: $paymentParams"
export tlsnProofSig=$(cast wallet sign --no-hash $paymentTypedHash --private-key=$tlsnWitnessPrivKey)
echo "tlsnProofSig: $tlsnProofSig"

cast send $TlsnProofVerifier \
  "releaseAfterProofVerify((uint256,address,uint256,uint256,uint256,address,address,uint256,bytes32,bytes32,bytes32,address,uint256),(bytes32,bytes32,bytes32,uint256,bytes32,uint64,uint256),bytes,bytes)" \
  "$escrowParms" \
  "$paymentParams" \
  "$tlsnProofSig" \
  "$relayerSig" \
...
echo "=========================================="
echo "Executing release after proof verify..."
echo "=========================================="
echo "TlsnProofVerifier Contract: $TlsnProofVerifier"
echo "Payer (EOA Buyer): $eoaBuyer"
echo "Executor (EOA Seller): $eoaSeller"
echo "Escrow Typed Hash (witness): $verifierEscrowTypedHash"
echo "Payment Typed Hash: $paymentTypedHash"
echo "Tlsn Proof Sig: $tlsnProofSig"
