...
# Arbitrator
if [ -z "$ARBITRATOR_PRIV_KEY" ]; then
    echo "Error: ARBITRATOR_PRIV_KEY environment variable is not set"
    exit 1
fi

if [ -z "$TBA_ARBITRATOR" ]; then
    echo "Error: TBA_ARBITRATOR environment variable is not set"
    exit 1
fi
...
export arbitratorPrivKey=$ARBITRATOR_PRIV_KEY
export tbaBuyer=$TBA_BUYER
export tbaSeller=$TBA_SELLER
export tbaArbitrator=$TBA_ARBITRATOR
export tradeId=${TRADE_ID:-1}
export chainId=$CHAIN_ID
export waypointName="MainnetWaypoint"
export waypointVersion="1"
export usdc=${USDC}
...
export json=$(cast send $TakeIntent \
  ...
  --gas-limit 5000000 --json)

echo "json: $json"
echo "tx hash: $(echo "$json" | jq -r '.transactionHash')"
export escrowHash=$(parse_log_data_field "$json" 'Created(address,address,address,bytes32,uint256,uint256)' 'FakeEvent(bytes32,uint256,uint256)' 1)
echo "escrowHash: $escrowHash"
...
export action1Selector=$(cast sig "DISPUTE_BY_BUYER((uint256,address,uint256,uint256,uint256,address,address,uint256,bytes32,bytes32,bytes32,address,uint256),bytes)")
export action1Params=$(cast abi-encode "x((uint256,address,uint256,uint256,uint256,address,address,uint256,bytes32,bytes32,bytes32,address,uint256),bytes)" "$escrowParms" "$relayerSig")
export action1Data="${action1Selector}${action1Params:2}"
export action1Hex=$(echo "$action1Data" | grep -q "^0x" && echo "$action1Data" || echo "0x$action1Data")
...
export counterpartySig=$(cast wallet sign --no-hash $resolvedResultTypedHash --private-key=$sellerPrivKey)
